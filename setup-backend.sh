...
# usage
if [ ! $# -eq 2 ] ; then
  echo -e "usage: $0 <gns3 proxy config file> <ip address of backend to configur                                                                                       e>, e.g.:\n"
...
sed s/"<BACKEND_IP_ADDRESS>"/"$BACKEND_IP_ADDRESS"/g config-templates/gns3_serve                                                                                       r.conf >config-templates/gns3_server.conf.$BACKEND_IP_ADDRESS
sed -i s/"<BACKEND_PORT>"/"$BACKEND_PORT"/g config-templates/gns3_server.conf.$B                                                                                       ACKEND_IP_ADDRESS
...
scp config-templates/gns3_server.conf.$BACKEND_IP_ADDRESS gns3@$BACKEND_IP_ADDRE                                                                                       SS:/home/gns3/.config/GNS3/2.2/gns3_server.conf
